...
if 'district' in df.columns and 'donation_total' in df.columns:
    ...
    if not df_local.empty:
        ...
        agg_df = df_local.groupby('district').agg(
            ...
            main_year=('donation_year', lambda x: x.mode()[0] if not x.mode().empty else '未知')
        ).reset_index()
        
        # 合併（left join）
        real_map_data = real_map_data.merge(agg_df, on='district', how='left')
        
        # 只在有真實總額時才更新（避免 KeyError）
        if 'donation_total_y' in real_map_data.columns:
            real_map_data['donation_total'] = real_map_data['donation_total_y'].combine_first(real_map_data['donation_total'])
            real_map_data = real_map_data.drop(columns=['donation_total_y'], errors='ignore')
